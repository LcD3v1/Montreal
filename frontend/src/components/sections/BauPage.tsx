...
import { useCreateBauLote } from '@/hooks/useBau'
import { useBauItens } from '@/hooks/useConfig'
...
export default function BauPage() {
  ...
  const podeMovimentar = canEditArea(user, 'bau')
  const { data: itens } = useBauItens()
  const { data: membros } = useMembros()
  const createLote = useCreateBauLote()
      ...
      <div className="p-6 max-w-2xl mx-auto">
        <GlowCard>
          <div className="p-8 text-center">
            ...
            <p className="font-mono text-xs text-txt2">Sua conta não tem permissão para movimentar o baú.</p>
          </div>
        </GlowCard>
      </div>
    )
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <GlowCard>
        <div className="p-6">
          <h2 className="font-orbitron text-sm font-bold text-gold tracking-wider mb-5">REGISTRAR MOVIMENTAÇÃO DO BAÚ</h2>
