...
import { useBauGerenciaEstoque } from '@/hooks/useBauGerencia'
import GlowCard from '@/components/ui/GlowCard'
import LoadingHud from '@/components/ui/LoadingHud'

export default function EstoqueGerenciaPage() {
  const { data: estoque, isLoading } = useBauGerenciaEstoque()
      ...
      <GlowCard>
        <div className="p-4">
          <h3 className="font-orbitron text-xs text-txt2 tracking-wider mb-4 flex items-center gap-2">
            <Package size={14} className="text-gold" /> ESTOQUE ATUAL DO BAÚ DA GERÊNCIA
          </h3>
            ...
            <tbody>
              {(estoque ?? []).length === 0 ? (
                ...
                <tr key={e.item} className="border-b border-bdr/50 hover:bg-bdr/30 transition-colors">
                  ...
                  <td className="px-4 py-3 font-orbitron text-sm" style={{ color: e.quantidade > 0 ? '#ffffff' : '#8a8a8a' }}>
                    {e.quantidade}
                  </td>
                </tr>
              ))}
