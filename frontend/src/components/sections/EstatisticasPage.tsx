...
export default function EstatisticasPage() {
  ...
  function generatePdf() {
    ...
    doc.text('SISTEMA MONTREAL', W / 2, y, { align: 'center' })
    y += 7
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`SISTEMA MONTREAL — ${now.toLocaleDateString('pt-BR')} — Pagina ${i} de ${pageCount}`, W / 2, 290, { align: 'center' })
    }
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={weekdayData}>
                ...
                <Bar dataKey="ações" radius={[3, 3, 0, 0]}>
                  {weekdayData.map((_, i) => (
                    <Cell key={i} fill={i === 0 || i === 6 ? '#8a8a8a' : '#CC0000'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            {qruExtData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={qruExtData}>
                  ...
                  <Bar dataKey="pos" name={L.pos} stackId="a" fill="#ffffff" />
                  <Bar dataKey="neg" name={L.neg} stackId="a" fill="#CC0000" radius={L.neu ? undefined : [3, 3, 0, 0]} />
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 rounded-full bg-bdrg overflow-hidden flex">
                          <motion.div style={{ background: '#ffffff' }} className="h-full" initial={{ width: 0 }} animate={{ width: `${q.total > 0 ? (q.pos / q.total) * 100 : 0}%` }} transition={{ duration: 0.7, delay: idx * 0.04 }} />
                          <motion.div style={{ background: '#CC0000' }} className="h-full" initial={{ width: 0 }} animate={{ width: `${q.total > 0 ? (q.neg / q.total) * 100 : 0}%` }} transition={{ duration: 0.7, delay: idx * 0.04 + 0.05 }} />
                        </div>
