...
if (!JWT_SECRET || JWT_SECRET.length < 32 || JWT_SECRET.includes('mude-para')) {
  console.error('[MONTREAL] ERRO FATAL: JWT_SECRET ausente ou fraco (mínimo 32 chars). Configure no .env.')
  process.exit(1)
}
...
if (existsSync(frontendDist)) {
  ...
  console.log('[MONTREAL] Servindo frontend de:', frontendDist)
} else {
  console.log('[MONTREAL] Frontend dist não encontrado — rode: npm run build')
}
...
ensureDefaultAdmin().then(() => {
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[MONTREAL] Sistema rodando em http://0.0.0.0:${PORT}`)
    console.log(`[MONTREAL] Ambiente: ${PROD ? 'produção' : 'desenvolvimento'}`)
  })
}).catch(err => {
  console.error('[MONTREAL] Erro ao inicializar:', err)
  process.exit(1)
})
